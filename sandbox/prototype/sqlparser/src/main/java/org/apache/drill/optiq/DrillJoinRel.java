...
public class DrillJoinRel extends JoinRelBase implements DrillRel {
  ...
  public DrillJoinRel(RelOptCluster cluster, RelTraitSet traits,
    ...
    case RIGHT:
      throw new InvalidRelException(
          "DrillJoinRel does not support RIGHT join");
    }
    RexNode remaining = RelOptUtil.splitJoinCondition(left, right, condition,
        leftKeys, rightKeys);
    if (!remaining.isAlwaysTrue()) {
      throw new InvalidRelException(
          "DrillJoinRel only supports equi-join");
    }
  }

  @Override
  public DrillJoinRel copy(RelTraitSet traitSet, RexNode condition,
...
/*
    E.g. {
      ...
      right: 4,
	    conditions: [
	      {relationship: "==", left: "deptId", right: "deptId"}
	    ]
	  }
*/
  ...
  private int implementInput(DrillImplementor implementor, int i, int offset,
      RelNode input) {
    ...
    final List<String> outputFields =
        fields.subList(offset, offset + inputFields.size());
  ...
  private int rename(DrillImplementor implementor, int inputId,
  ...
  /** Returns whether there are any elements in common between left and
   * right. */
  ...
  private boolean uniqueFieldNames(RelDataType rowType) {
    return isUnique(RelOptUtil.getFieldNameList(rowType));
  }
